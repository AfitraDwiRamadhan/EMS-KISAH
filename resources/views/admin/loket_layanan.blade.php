...
<div class="container-fluid px-4 py-3" style="min-height: 100vh;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            <button type="button" class="btn btn-purple fw-bold px-3 py-2 text-white border-0 shadow-sm rounded-3" data-bs-toggle="modal" data-bs-target="#modalBulkPasien" style="background-color: #6f42c1;">
                <i class="fa-solid fa-bolt-lightning me-2"></i> Smart Parser (Warga Log)
            </button>
            <button type="button" class="btn btn-primary fw-bold px-3 py-2 text-white border-0 shadow-sm rounded-3" data-bs-toggle="modal" data-bs-target="#modalTambahLoket">
    ...
    <div class="row g-4">
            ...
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content border-0 shadow">
                    <div class="modal-header bg-dark text-white border-0 py-3">
                        <h6 class="modal-title fw-bold">Pengaturan Loket</h6>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <form action="{{ route('admin.loket.updateKategori', $kat->id) }}" method="POST">
                        @csrf @method('PUT')
                        <div class="modal-body p-4 bg-grey">
                        ...
                        <div class="modal-footer border-0 bg-grey py-2">
        ...
        <div class="modal fade" id="modalAntrean{{ $kat->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content border-0 shadow">
                    <div class="modal-header bg-dark text-white border-0 py-3">
                        <h5 class="modal-title fw-bold"><i class="fa-solid fa-users text-primary me-2"></i>Antrean Pasien: {{ $kat->nama_layanan }}</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                            ...
                            <tbody>
                                ...
                                <tr class="border-bottom border-light-subtle">
                                    ...
                                    <td class="text-center pe-4">
                                        <button type="button" class="btn btn-sm btn-primary fw-bold px-3" data-bs-toggle="modal" data-bs-target="#modalLihatForm{{ $row->id }}">
...
<div class="modal fade" id="modalBulkPasien" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-dark text-white border-0 py-3">
                <h5 class="modal-title fs-6 fw-bold"><i class="fa-solid fa-wand-magic-sparkles text-warning me-2"></i>Smart Bulk Parser Antrean Warga</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('admin.loket.storeBulk') }}" method="POST">
                @csrf
                <div class="modal-body p-4 bg-gray">
                    <div class="alert alert-secondary py-2 small border-0 shadow-sm d-flex align-items-center mb-3 bg-white">
...
<div class="modal fade" id="modalLihatForm{{ $row->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-dark text-white border-0 py-3">
                <h6 class="modal-title fw-bold"><i class="fa-solid fa-clipboard-list text-warning me-2"></i>Berkas Pasien: {{ $row->nama_pasien }}</h6>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4 bg-grey">
                ...
                <div class="card p-3 border-secondary-subtle mb-4 shadow-sm bg-gray-subtle" style="border-radius: 8px;">
                    <table class="table table-sm table-borderless mb-0" style="font-size: 0.85rem;">
                        <tbody>
                            @if(is_array($row->data_lengkap) || is_object($row->data_lengkap))
                                @foreach($row->data_lengkap as $kunci => $nilai)
                                <tr>
                                    <td width="45%" class="text-muted fw-bold text-uppercase" style="font-size: 0.73rem;">{{ $kunci }}</td>
                                    <td width="5%" class="text-muted">:</td>
                                    <td class="fw-semibold text-dark">{{ $nilai ?? '-' }}</td>
                                </tr>
                                @endforeach
                            @else
                ...
                <form action="{{ route('admin.loket.updateStatus', $row->id) }}" method="POST" class="p-3 bg-gray-subtle rounded border">
                    @csrf @method('PUT')
                    <label class="form-label small fw-bold text-dark mb-2">Progres Penanganan Medis:</label>
